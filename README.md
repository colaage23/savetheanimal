...
#### 오류 및 어려웠던 점
  - 프로젝트를 진행하면서 기능 구현에 필요한 데이터를 가져오는 코드를 작성하지 않아 컨트롤러를 하나 더 거쳐야 하는 번거로움이 있었고, 기획과 순서의 중요성을 알게 되었다.
  - 즐겨찾기 기능을 구현할 때, 레시피를 검색해서 상세페이지로 이동하면 즐겨찾기가 활성화 되지 않는 문제가 있었으나 이를 해결하지 못하고 프로젝트를 마친게 아쉬웠다.
---
